...
function get() {
    ...
    if(full_screen) {
        if(window.navigator.userAgent.includes("Tizen"))
            $("#time").style.fontSize = hrs ? "20vw" : "31vw";
        else
...
function setSchedule(...args) {
    ...
    if(args[0] == "custom") {
        ...
        $("#alt_format").textContent = "Toggle Format";
        $("#hider").style.display = ""
        $("#alt_format").style.display = ""
    } else {
        ...
        $("#alt_format").textContent = "";
        $("#alt_format").style.display = "none";
    }
...
if(just_now.getDay() == 6 || just_now.getDay() == 0) {
    ...
    fetch("https://cors-container.herokuapp.com/https://www.d125.org/cf_calendar/feed.cfm?type=ical&feedID=E96D4A2A781C43699D5A4645042A0F79").then((resp) => {
